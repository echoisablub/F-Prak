...
$$E(t)=E_1(t)+E_2(t)$$
Die resultierende Intensität der Überlagerung sieht in unserem Fall von zwei gleichen Wellen folgendermaßen aus\cite{vmappe}
\begin{equation}
I(\Delta t)=<(E_1+E_2)(E_1+E_2)^*>=I_1+I_2+2Re<(E^*_1E_2)>=I_1+I_1+2Re\Gamma(\Delta t)
\end{equation}
Der dritte Term $\Gamma(\Delta t)$ beinhaltet die Intensität des korrelierten Teils der Überlagerung und nennt sich (Selbst-) Kohärenzfunktion oder auch Autokorrelationsfunktion, weil sie der Autokorrelation des lichtfeldes $E_1(t)$ entspricht\cite{vmappe}
\begin{equation}
...
In der experimentellen Praxis ist die Kohärenzfunktion $\Gamma(\Delta t)$ nicht direkt zugänglich. Stattdessen wird der Kontrast $K$ (oder Sichtbarkeit) des Interferenzmusters herangezogen, definiert über die Intensitätsmaxima und -minima\cite{vmappe}
\begin{equation}
...
\end{equation}
Unter der Annahme einer symmetrischen spektralen Energiedichte ($W(\omega) = W(-\omega)$) ergibt sich für den interferierenden Teil der Intensität $\Delta I(\Delta t)$ nach dem Wiener-Khinchin-Theorem der Zusammenhang [14-16]:
\begin{equation}
\Delta I(\Delta t) = \frac{1}{2\pi} \int_{-\infty}^{\infty} W(\omega) e^{-i\omega \Delta t} d\omega
\end{equation}
Durch eine numerische Rücktransformation (Inverse Fourier-Transformation) kann somit aus dem gemessenen Interferogramm das gesuchte Spektrum extrahiert werden [16-18]:
\begin{equation}
W(\omega) = \int_{-\infty}^{\infty} \Delta I(\Delta t) e^{i\omega \Delta t} d(\Delta t)
\end{equation}

In der Realität ist die spektrale Auflösung $\Delta\nu$ jedoch durch die maximale zeitliche Verzögerung $\Delta t_{max}$ begrenzt, die experimentell durch den Verfahrweg des Spiegels erreicht werden kann: $\Delta\nu = 1/\Delta t_{max}$ [17, 19, 20]. Zudem begrenzt das Nyquist-Shannon-Abtasttheorem die maximal auflösbare Frequenz $\nu_{max}$ durch das Zeitintervall $t_{sampl}$ zwischen zwei Messpunkten: $\nu_{max} = 1/(2 t_{sampl})$.
...
\begin{itemize}
       ...
       \item Erzeugung eines äquidistanten Gitters: Eine zwingende mathematische Voraussetzung für die Durch-führung der diskreten Fouriertransformation (FFT) ist eine streng äquidistante Abtastung des Signals. Da die Ortsachse nach der Addition der Korrekturwerte zwar linearisiert, aber die Abstände zwischen den Punkten nun ungleichmäßig verteilt sind, müssen die Datenpunkte neu angeordnet werden.
...
Die theoretische Auflösungsgrenze des Fourier-Transform-Spektrometers lässt sich bestimmen durch die Wegzeitdifferenz $\Delta t$. Da der benutzte Schrittmotor eine kleinstmögliche Verschiebung von 50,5nm hat und wir 10000 Schritte pro Messreihe gelaufen sind, ist unsere Weglängendifferenz $\Delta s = 10000 \cdot 50,5 \cdot 10^{-9} \cdot 2 = 1,01$mm.
Daraus lässt sich $\Delta t = \frac{1,01 \cdot 10^{-3}}{c} = 3,37$pm bestimmen.\\
Die theoretische spektrale Auflösungsgrenze ist dann:
$$\Delta\nu=\frac{1}{\Delta t}=0,297\text{PHz}=1,009\cdot 10^{-3}\text{nm}$$
\\
Experimentell lässt sich eine Signalbreite von 0,6 PHz aus dem Laserspektrum ablesen. Die Auflösung im Experiment ist 50\% größer als die theoretische Auflösungsgrenze.
...
Einer der größten systematischen Fehler in diesem Experiment ist die Nichtlinearität des Schrittmotors. Für diesen Fehler wird in der Analyse der Daten durch die Korrekturfunktion gegengesteuert. Da die Spektren den theoretischen Erwartungen entsprechen, ist davon auszugehen, dass die Korrektur gut gelangen ist. Diese Annahme wird auch von dem Vergleich des Lasersignals mit der korrigierten Ortsachse zu dem ursprünglich interpolierten Signal unterstützt (siehe \autoref{fig: korrvgl}).
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.75\linewidth]{img/Vergleich Ursprüngliche vs Korrigierte Interpolation.png}
...
\end{figure}
\\\\
Es wurden für jede Probe zwei Messreihen aufgenommen. Die Ergebnisse der jeweils ersten Messreihen entsprechen den theoretischen Erwartungen weniger gut als die Ergebnisse der zweiten Messreihen und deuten auf einen beachtlichen systematischen Fehler hin, da die Auswertungsmethode die gleiche für alle Datensätze ist. In der oberen Auswertung sind die Ergebnisse der zweiten Messreihen zu sehen.\\
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.75\linewidth]{img/Spektrum Filter.png}
	\caption{Das in Messreihe 1 gemessene Spektrum des Filters in Abhängigkeit von Frequenz und Wellenlänge dargestellt und verglichen mit dem Referenzspektrum der Weißlichtquelle}
	\label{fig: filter1spek}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.75\linewidth]{img/Spektrum Iod.png}
	\caption{Das in Messreihe 1 gemessene Iod-Spektrum in Abhängigkeit von Frequenz und Wellenlänge dargestellt und verglichen mit dem Referenzspektrum der Weißlichtquelle gemessen durch eine leere Küvette}
	\label{fig: iod1spek}
\end{figure}
Bei den ersten Messreihen sind die Intensitäten und das Rauschen der Spektren größer (siehe \autoref{fig: filter1spek} und \autoref{fig: iod1spek}). Zwischen den beiden Messreihen wurde das Spektrometer neu justiert und die Nullposition des Motors neu eingestellt. Es lässt sich schließen, dass die erste Justage qualitativ leicht schlechter war. Außerdem ist zwischen erster Justage und ersten Messreihen mehr Zeit vergangen als zwischen zweiter Justage und zweiten Messreihen, was heißt, dass das Spektroskop mehr Zeit hatte sich aufgrund von externen Faktoren, wie Temperaturschwankungen im Raum, kleine Vibrationen des Tisches und das Rein- und Rausgehen in und aus dem Raum um Proben auszutauschen, zu ändern.
...
\begin{itemize}
       ...
       \item Die experimentell bestimmte spektrale Auflösung lag mit ca. 0,6 PHz in der Größenordnung der theoretischen Auflösungsgrenze von 0,297 PHz.
\end{itemize}
