...
Bei Fourier-Transform-Spektroskopie werden Spektren durch Messungen der Kohärenz der Strahlungsquelle bestimmt. Dafür wird die Intensität in Abhängigkeit der Motorposition gemessen, die Motorpositionen in eine Zeitachse umgerechnet, und das Spektrum durch die namensgebende Fouriertransformation berechnet.\\
...
\begin{figure}[H]
       \centering
       \includegraphics[width=1\linewidth]{img/Laser Daten Interpolation vs Originaldaten_neu.png}
...
\begin{figure}[H]
       \centering
       \includegraphics[width=1\linewidth]{img/laser interpolation mit maxima_neu.png}
...
\begin{figure}[H]
       \centering
       \includegraphics[width=1\linewidth]{img/Kalibrierfunktion_neu.png}
...
\begin{figure}[H]
       \centering
       \includegraphics[width=1\linewidth]{img/Korrektur der Motorposition_neu.png}
...
\end{figure}
Das Spektrum des Interferenz-Filters weicht hauptsächlich in den Intensitäten von dem Referenzspektrum ab (siehe \autoref{fig: filter2spek}). Die genaueren Abweichungen lassen sich an der differentiellen optischen Transmission des Filters erkennen (siehe \autoref{fig: filter2dot}).\\
\begin{figure}[H]
	\centering
...
\end{figure}
Auffällig sind hier vor allem die starke Absorption in einem Wellenlängenbereich von etwa \qtyrange{506}{521}{\nm}. Der Filter filtert also grünes Licht in einem schmalen Bereich, was für einen schmalbandigen Interferenz-Filter spricht.\\\\
Die spektrale Bandbreite $B$ des Filters lässt sich mithilfe der DOT in Frequenzdarstellung bestimmen:
$$B=f_{max}-f_{min}=\frac{c}{\lambda_{min}}-\frac{c}{\lambda_{max}}=\frac{c}{506\si{\nm}}-\frac{c}{521\si{\nm}}=592,5\si{\tera\hertz}-575,4\si{\tera\hertz}=\qty{17,0}{\tera\hertz}$$
Die Zentralwellenlänge $\lambda_Z$ liegt bei $\lambda_Z=\frac{c}{f_{min}+\frac{B}{2}}=\qty{513,4}{\nm}$.
...
\begin{figure}[H]
	...
	\label{fig: iod2od}
\end{figure}
Bei der Betrachtung des Iod-Spektrums (siehe \autoref{fig: iod2spek}) ist auffällig, dass bei der Messung des Iod-Spektrums deutlich weniger von dem Signal angekommen ist, als bei der Aufnahme des Interferenz-Filter-Spektrums. Dadurch ist das Rauschen deutlich prominenter zu sehen.\\
Die gemessene Intensität ist außerdem bei dem Iod-Spektrum in einem Wellenlängenbereich von ca. \qtyrange{500}{650}{nm} deutlich geringer als das Referenzspektrum, da Iod im Allgemeinen kurzwelliges Licht stärker absorbiert, als langwelliges Licht.
\\\\
...
Daraus lässt sich $\Delta t = \frac{1,01 \cdot 10^{-3}}{c}\si{m} = \qty{3,37}{\ps}$ bestimmen.\\
Die theoretische spektrale Auflösungsgrenze ist dann:
$$\Delta\nu=\frac{1}{\Delta t}=\qty{0,297}{\tera\hertz}$$
\\
Experimentell lässt sich eine Signalbreite von \qty{0,6}{\tera\hertz} aus dem Laserspektrum ablesen. Die Auflösung im Experiment ist doppelt so groß, wie die theoretische Auflösungsgrenze.
...
%Die Interpolation der Laserdaten zum Aufbauen der Kalibrierfunktion ist eine der grundlegenden, da das Signal des Lasers Schwankungen hat, die sich durch alle restlichen Ergebnisse fortpflanzen. Das würde auch heißen, dass die Linienbreite des Lasers größer ist als angenommen, das Experiment hat also eine schlechtere Auflösung, als jetzt bestimmt.\\ 
...
Für jede Probe wurden jeweils zwei Messreihen durchgeführt (siehe \autoref{sec: messreihe anhang}). Die diskutierten Ergebnisse sind die, der ersten Messreihe, da diese mehr Fehler aufweist als die zweite.\\Vor der zweiten Messreihe wurde das Michelson-Interferometer neu adjustiert, was zu einem saubereren Filter-Spektrum und einer klareren differentiellen Transmission führt. Das Iod-Spektrum ist ähnlich verrauscht, weswegen es hier nicht weiter ausgeführt wird.

%Zur Kompensation dieses Fehlers wird in der Datenanalyse eine Kalibrierfunktion angewendet. Da die Spektren grundsätzlich den theoretischen Erwartungen entsprechen, ist davon auszugehen, dass die Korrektur erfolgreich durchgeführt wurde.\\
...
\begin{itemize}
       \item Für den Interferenzfilter wurde eine Zentralwellenlänge von ca. \qty{513,4}{\nm} bei einer spektralen Bandbreite von \qty{17,0}{\tera\hertz} ermittelt.
       ...
       \item Die experimentell bestimmte spektrale Auflösung lag mit ca. \qty{0,6}{\tera\hertz} in der Größenordnung der theoretischen Auflösungsgrenze von \qty{0,297}{\tera\hertz}.
\end{itemize}
...
\begin{appendices}
\section{Code}
\lstinputlisting[language=Python]{KV6 Aufgeraumt.py}

\section{Messreihe 2}\label{sec: messreihe anhang}
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/Spektrum Laser 2 final.png}
	\caption{Zwei Ausschnitte des Laserspektrums jeweils abhängig von Frequenz oder Wellenlänge.}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/Spektrum filter 2 final.png}
	\caption{Das gemessene Spektrum des Filters in Abhängigkeit von Frequenz und Wellenlänge dargestellt und verglichen mit dem Referenzspektrum der Weißlichtquelle.}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/Differentielle Transmission Filter 2 final.png}
	\caption{Die differentielle Transmission des Filter-Spektrums bestimmt durch das Referenzspektrum der Weißlichtquelle.}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/Spektrum Iod 2 final.png}
	\caption{Das gemessene Iod-Spektrum in Abhängigkeit von Frequenz und Wellenlänge dargestellt und verglichen mit dem Referenzspektrum einer leeren Küvette.}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/Absorsptionsprofil Iod 2 OD final.png}
	\caption{Die optische Dichte des Iod-Spektrums bestimmt durch das Referenzspektrum einer leeren Küvette.}
\end{figure}
