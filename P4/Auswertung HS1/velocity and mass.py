...
data_new = np.column_stack((data, v_R, v_R_err, M_R, M_R_err))
filepath_newdata="Auswertung HS1/calibrated 21cm/cm21_tab_newdata.txt"
np.savetxt(
    filepath_newdata,
    data_new,
    header="l[deg] R[kpc] R_err v_max[kms-1] v_max_err v_R[kms-1] M_R[M_sun], M_R_err",
    comments="",
    fmt=["%.0f", "%.2f", "%.2f", "%.2f", "%.2f", "%.2f", "%.2f", "%.2f", "%.2f"]
)
