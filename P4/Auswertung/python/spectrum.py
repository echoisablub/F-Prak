...
for i in observations:
    left = left + scidata[i*3][7] - scidata[i*3+1][7]
    right = right + scidata[i*3][8] - scidata[i*3+1][8]
    left_obs = scidata[i*3][7] - scidata[i*3+1][7]
    right_obs = scidata[i*3][8] - scidata[i*3+1][8]
    
    print('On-Off Messung:', "{:02d}".format(i+1), 'rms_left =', int(round(np.std(cal_left * len(observations) * left_obs[line_free] - baseline_subtract))), 'K; rms_right = ', int(round(np.std(cal_right * len(observations) * right_obs[line_free] - baseline_subtract))), 'K')
print('======================================================')    

mean_left = np.mean(left[365:380])
...
#    freq_right.append(1397.55 + (i-1)*62.5/1024)
    freq_left.append(1397.57 + i*62.5/1024)
...
for i in range(0, 1024):
    ...
    if i in rfi_left or i+1 in rfi_right:
