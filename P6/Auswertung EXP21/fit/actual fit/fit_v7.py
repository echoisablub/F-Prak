...
def fit_difference_spectrum(energy, difference, sigma, reference_spectra, fixed_amplitude=None):
    ...
    if fixed_amplitude is None:
        ...
        bounds = [(0.0, None)] + [(0.0, 1.0)] * 4
        population_parameters = lambda parameters: parameters[1:]
    else:
    ...
    amplitude = result.x[0] if fixed_amplitude is None else fixed_amplitude
...
def fit_amplitude_for_pure_quintet(difference, sigma, reference_spectra):
    ...
    if denominator <= 0.0:
        raise ValueError("Das Quintett-Referenzspektrum ist für die A-Kalibrierung ungeeignet.")
...
def fit_singlet_transition(time_fs, singlet_population, singlet_uncertainty):
    ...
    t0 is the midpoint of the population drop. sigma_time is the standard
    deviation of the Gaussian temporal response, not the FWHM.
    """
    ...
    initial_parameters = [
        np.clip(singlet_population[-1], 0.0, 1.0),
        np.clip(singlet_population[0] - singlet_population[-1], 0.0, 1.0),
        time_fs[np.argmin(np.abs(singlet_population - 0.5))],
        max(np.ptp(time_fs) / 5.0, 1.0),
    ]
    parameter_bounds = (
        [0.0, 0.0, np.min(time_fs) - np.ptp(time_fs), 1e-6],
        [1.0, 1.0, np.max(time_fs) + np.ptp(time_fs), 10.0 * np.ptp(time_fs)],
    )
